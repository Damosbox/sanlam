  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: claims } = await supabase
        .from("claims")
        .select("assigned_broker_id, cost_estimation")
      ...
      for (const s of subs || []) {
        ...
        premiumMap.set(s.assigned_broker_id, (premiumMap.get(s.assigned_broker_id) || 0) + (Number(s.monthly_premium) || 0));
      }

      const claimMap = new Map<string, { total: number; count: number }>();
      for (const c of claims || []) {
        if (!c.assigned_broker_id) continue;
        const cur = claimMap.get(c.assigned_broker_id) || { total: 0, count: 0 };
        cur.total += Number(c.cost_estimation) || 0;
        cur.count++;
        claimMap.set(c.assigned_broker_id, cur);
      }
      ...
      const result: AgentLossRatio[] = brokerIds.map((id) => {
        ...
        return {
          ...
          lossRatio: premiums > 0 ? (claimData.total / premiums) * 100 : 0,
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold">{formatFCFA(totalPremiums)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent><div className="text-2xl font-bold">{formatFCFA(totalClaims)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${globalRatio > THRESHOLD ? "text-destructive" : "text-primary"}`}>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Agents &gt; {THRESHOLD}%</CardTitle>
            <AlertTriangle className="h-4 w-4 text-destructive" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{aboveThreshold}</div>
          </CardContent>
        </Card>
      </div>
              ...
              <BarChart data={chartData}>
                ...
                <Bar dataKey="ratio" radius={[4, 4, 0, 0]}>
                  {chartData.map((entry, i) => (
                    <Cell key={i} className={entry.ratio > THRESHOLD ? "fill-destructive" : "fill-primary"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">Nb Sinistres</TableHead>
              ...
              {data.map((a) => (
                <TableRow key={a.agentId}>
                  ...
                  <TableCell className="text-center">
                    <Badge variant={a.lossRatio > THRESHOLD ? "destructive" : "secondary"}>
                      {a.lossRatio.toFixed(1)}%
                    </Badge>
                  </TableCell>
                </TableRow>
              ))}
